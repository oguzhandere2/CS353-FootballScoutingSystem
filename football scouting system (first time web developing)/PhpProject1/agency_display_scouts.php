              ...
              <tbody class="text-white">
                  <?php
                    ...
                    $userid = $_SESSION['loggedInUserID'];
                     ...
                     $result = mysqli_query($db, "select * from scout where scouting_agency_user_id = '$userid' order by reputation ") or die("Unable to access" . mysqli_error());
           
                      while ($row = mysqli_fetch_assoc($result)) 
                      {
                          
                      $scout_id = $row['user_id'];  
                      $result3 = mysqli_query($db, "select * from user where id = '$scout_id' ") or die("Unable to access" . mysqli_error());
                      $row2 = mysqli_fetch_array($result3);
                      $scout_name = $row2['name'];   

                      $result2 = mysqli_query($db, "select * from scout where user_id = '$scout_id' order by reputation ") or die("Unable to access" . mysqli_error());
                      $row3 = mysqli_fetch_array($result2);
                      
                      $scout_nationality = $row['nationality'];
                      $scout_age = $row3['age'];
                      $scout_reputation = $row3['reputation'];
